...
abstract class BasesfGuardUserProfile extends BaseObject  implements Persistent {
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUserProfile has previously been saved, it will retrieve
	 * related AccessLogs from storage. If this sfGuardUserProfile is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collAccessLogs === null) {
			if ($this->isNew()) {
			   $this->collAccessLogs = array();
				...
				$criteria->add(AccessLogPeer::USER_ID, $this->user_id);
	 ...
	 * actually need in sfGuardUserProfile.
	 */
	public function getAccessLogsJoinAsset($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserProfilePeer::DATABASE_NAME);
		}
		...
		if ($this->collAccessLogs === null) {
			if ($this->isNew()) {
				...
				$this->collAccessLogs = AccessLogPeer::doSelectJoinAsset($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
